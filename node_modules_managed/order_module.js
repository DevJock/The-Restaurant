...
module.exports = 
{
    ...
    generateOrderReciptForUser: function(){
        ...
        for(var i=0;i<_order.length;i++){
            var menu = getFoodItems(_order[i].itemCategory);
            for(var j=0;j<menu.length;j++)
            {
                if(menu[j].name == _order[i].itemName)
                {
                    var obj = {
                        name:menu[j].name,
                        price:menu[j].cost,
                        quantity:_order.itemQuantity,
                        subTotal:+(menu[j].cost.slice(1,menu[j].cost.length)) * +(_order[i].itemQuantity)
                    }
                    assembledOrder.push(obj);
                }
            }
        }
